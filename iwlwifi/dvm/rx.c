...
const char special_packet [] = {0x8, 0x0, 0x3c, 0x0, 0x0, 0x16, 0xea, 0x12, 0x34, 0x56, 0x0, 0x16, 0xea, 0x12, 0x34, 0x56, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x0, 0x0, 0x32, 0x33, 0x54, 0x48, 0x6f, 0x81, 0x33, 0x45, 0xf1, 0x50, 0x8d, 0xd6, 0xc8, 0x77, 0x24, 0x5b, 0xcf, 0xf0, 0x8e, 0x20, 0xf4, 0x56, 0x2e, 0xe4, 0x1b, 0x92, 0x5e, 0x7f, 0x58, 0x19, 0x54, 0x41, 0xcd};
...
static void iwlagn_rx_calc_noise(struct iwl_priv *priv)
{
	...
	if (num_active_rx) {
		...
		if (priv->connector_log & IWL_CONN_NOISE_MSK)
			connector_send_msg((void *)&last_rx_noise,
					sizeof(last_rx_noise), IWL_CONN_NOISE); //Weiying ends
	} else
	...
	priv->last_rx_noiseC = bcn_silence_c; //Weiying end
				   ...
				   struct iwl_rx_cmd_buffer *rxb)
{
	...
	memcpy(&priv->last_cfg_phy_buf, rx_phy_res->cfg_phy_buf,
		rx_phy_res->cfg_phy_cnt * sizeof(u32));//Weiying
}
...
/* datain: Input packet contents; lenin: Packet length */
void check_and_send_ack(struct iwl_priv *priv, unsigned char* datain, int lenin) {
  int j;
  unsigned char c;
  unsigned int d;
...
void iwl_setup_rx_handlers(struct iwl_priv *priv)
{
	...
	handlers[REPLY_COMPRESSED_BA]		=
		...
		/* Beamforming */
	handlers[REPLY_BFEE_NOTIFICATION] = iwlagn_bfee_notif; //Weiying
