...
void iwlagn_bfee_notif(struct iwl_priv *priv, struct iwl_rx_cmd_buffer *rxb)
{
	 ...
	 * from SVD
	 */
	 //Weiying start, add sec and micro seconed as epoch time
	struct iwl_rx_packet *pkt = rxb_addr(rxb);
	struct iwl_bfee_notif_legacy *bfee_notif_legacy = (void *)pkt->data;
	struct iwl_bfee_notif* bfee_notif;
	struct timeval cur_tval;
	u8 Nrx, Ntx;
	u16 len = le16_to_cpu(bfee_notif_legacy->len);
	struct iwl_rx_phy_res *phy;
	u32 *non_cfg_buf, *cfg_buf;
	static u16 bfee_count;
	u16 last_seen;
	bfee_notif = kmalloc(len+sizeof(struct iwl_bfee_notif), GFP_ATOMIC);
	do_gettimeofday(&cur_tval);
	memcpy(bfee_notif->payload, bfee_notif_legacy->payload,len);
	bfee_notif->len = bfee_notif_legacy->len;
	bfee_notif->reserved1 = bfee_notif_legacy->reserved1;
	bfee_notif->Nrx = bfee_notif_legacy->Nrx;
	bfee_notif->Ntx = bfee_notif_legacy->Ntx;
	if(priv->csi_valid_mac) {
          memcpy(bfee_notif->macaddr, priv->csi_last_mac, ETH_ALEN*sizeof(u8));
        } else {
          memset(bfee_notif->macaddr, 0, ETH_ALEN*sizeof(u8));
        }
	//Weiying End
